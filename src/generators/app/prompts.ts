...
export async function askUseLastConfig(): Promise<boolean> {
...
export async function askBaseProjectInfo(
  initializr: SpringMetadata,
...
export async function askPackageName(defaultPackageName: string): Promise<string> {
...
export async function askReuseLastDeps(): Promise<boolean> {
...
export async function askAddMoreDeps(): Promise<boolean> {
...
export function createSearchDependencies(
  allDependenciesFlat: { name: string; value: string }[],
  selectedDependencies: { id: string; name: string }[]
) {
  return async (_: any, input: string = "") => {
    ...
    return allDependenciesFlat
      .filter(
        (dep) =>
          !selectedDependencies.find((d) => d.id === dep.value) &&
...
export async function askDependencies(
  allDependenciesFlat: { name: string; value: string }[],
  selectedDependencies: { id: string; name: string }[]
): Promise<{ id: string; name: string }[]> {
  const searchDependencies = createSearchDependencies(allDependenciesFlat, selectedDependencies);

  const showSelectedDependencies = () =>
    selectedDependencies.map((d) => `- ${chalk.cyan(d.name)}`).join("\n  ");
  ...
  while (!done) {
    ...
    const depMatch = allDependenciesFlat.find(
      (dep) =>
        dep.value.toLowerCase() === trimmed.toLowerCase() ||
